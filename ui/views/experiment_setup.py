...
def render(outputs_root: Path) -> None:
    ...
    section = st.radio(
        ...
        format_func=lambda k: (
            "Load Existing Dataset" if k == "load" else "Generate New Dataset"
        ),
        horizontal=True,
        key="setup_section",
    )
...
def _render_load(outputs_root: Path) -> None:
    ...
    if setup_arts:
        probe = [a for a in setup_arts if a.category == CATEGORY_PROBE]
        behavior = [a for a in setup_arts if a.category == CATEGORY_BEHAVIOR]
        if probe:
            st.markdown("### Probe trajectory — Neuropixels insertion anatomy")
            render_artifact_gallery(
                probe,
                columns=1,
                key="setup_load_probe",
                show_pdf_captions=True,
            )
        if behavior:
            st.markdown("### Behavior — locomotor covariates and neural drivers")
            render_artifact_gallery(
                behavior,
                columns=1,
                key="setup_load_behavior",
                show_pdf_captions=True,
            )
    else:
...
def _render_generate(outputs_root: Path) -> None:
    ...
    if st.button("Generate Dataset", type="primary", disabled=bool(errs)):
        st.session_state["setup_section"] = "generate"
            ...
            if setup_arts:
                ...
                if behavior:
                    ...
                    render_artifact_gallery(
                        behavior,
                        columns=1,
                        key="gen_done_behavior",
                        show_pdf_captions=True,
                    )
            st.info(
                "Dataset ready for analysis. Continue to **Neural Simulation**, "
                "**Feature Construction**, **Latent Representations**, or **Decoder Benchmark**."
            )
