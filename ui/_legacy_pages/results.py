...
from ui.components.controls import dataset_selector
...
def render(outputs_root: Path) -> None:
    ...
    dataset = dataset_selector(outputs_root, key="res_dataset")
    ...
    if dataset is None:
        st.info("Select a dataset to browse results.")
        return
    ...
    if artifacts_cmp.feature_set_summary is not None:
        with st.expander("Feature-set performance summary"):
            st.dataframe(artifacts_cmp.feature_set_summary, use_container_width=True, hide_index=True)
    if artifacts_cmp.loss_summary is not None:
        with st.expander("Sorted information-loss summary"):
            st.dataframe(artifacts_cmp.loss_summary, use_container_width=True, hide_index=True)
